...
const rgxFindMatchPos = function (str, left, right, flags) {
  ...
  const f = flags || '';
    ...
    const pos = [];
    let t; let s; let m; let start; let end;
...
showdown.subParser('autoLinks', function (text, options, globals) {
  ...
  const simpleURLRegex  = /\b(((https?|ftp|dict):\/\/|www\.)[^'">\s]+\.[^'">\s]+)()(?=\s|$)(?!["<>])/gi;
      const simpleURLRegex2 = /\b(((https?|ftp|dict):\/\/|www\.)[^'">\s]+\.[^'">\s]+?)([.!?()]?)(?=\s|$)(?!["<>])/gi;
      const delimUrlRegex   = /<(((https?|ftp|dict):\/\/|www\.)[^'">\s]+)>/gi;
      const simpleMailRegex = /(?:^|\s)([A-Za-z0-9!#$%&'*+-/=?^_`{|}~.]+@[-a-z0-9]+(\.[-a-z0-9]+)*\.[a-z]+)(?:$|\s)/gi;
  ...
  function replaceMail(wholeMatch, mail) {
    const unescapedStr = showdown.subParser('unescapeSpecialChars')(mail);
...
showdown.subParser('headers', function (text, options, globals) {
  ...
  function headerId(m) {
    let title; const escapedId = m.replace(/[^\w]/g, '').toLowerCase();
...
showdown.subParser('tables', function (text, options, globals) {
  ...
  text = text.replace(tableRgx, function (rawTable) {
    ...
    const rawHeaders = tableLines[0].split('|').map(function (s) { return s.trim();});
        const rawStyles = tableLines[1].split('|').map(function (s) { return s.trim();});
        const rawCells = [];
        const headers = [];
        const styles = [];
        const cells = [];
